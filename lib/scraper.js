...
async function scrapeChapter(version, book, chapter) {
  ...
  $('p').each((i, el) => {
    ...
    // Paragraf yang lolos ke fallback "!title && !content" tapi ternyata
    // tidak punya nomor ayat valid (verse tetap 0) itu bukan isi ayat -
    // biasanya footer/copyright/disclaimer dari halaman sumber. Buang.
    if (type === 'content' && verse === 0) {
      type = null;
    }
